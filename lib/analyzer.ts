import { ParsedInstruction } from './assembler';

export interface AnalyzerWarning {
  line: number;
  message: string;
  type: 'unused_label' | 'infinite_loop' | 'missing_hlt' | 'uninitialized_sp' | 'stack_underflow' | 'register_overwrite' | 'memory_overwrite' | 'no_termination';
}

export class Analyzer8085 {
  static analyze(instructions: ParsedInstruction[], labels: Record<string, number>): AnalyzerWarning[] {
    ...
    const registerState: Record<string, 'written' | 'read' | 'empty'> = {
      A: 'empty', B: 'empty', C: 'empty', D: 'empty', E: 'empty', H: 'empty', L: 'empty'
    };

    let lastMemoryWriteLine: number | null = null;
    ...
    instructions.forEach((inst, index) => {
      ...
      if (['JMP', 'JC', 'JNC', 'JZ', 'JNZ', 'JM', 'JP', 'JPE', 'JPO', 'CALL', 'CC', 'CNC', 'CZ', 'CNZ', 'CM', 'CP', 'CPE', 'CPO'].includes(op)) {
        if (inst.args[0]) usedLabels.add(inst.args[0]);
      }
      ...
      if (op === 'MVI' || op === 'LXI') {
        ...
        if (reg) registerState[reg] = 'written';
        if (reg === 'B') registerState['C'] = 'written'; // LXI B sets B and C
        if (reg === 'D') registerState['E'] = 'written';
        if (reg === 'H') registerState['L'] = 'written';
      }
