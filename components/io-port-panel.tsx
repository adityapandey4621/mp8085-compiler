...
export default function IOPortPanel({ ports }: IOPortPanelProps) {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 animate-slide-left">
      {/* Input Ports */}
      <div className="bg-[#0a0a0f]/90 border border-[#1a1a2e] rounded-2xl overflow-hidden backdrop-blur-xl">
        <div className="flex items-center gap-2 px-3 py-2 border-b border-[#1a1a2e] bg-[#0d0d12]">
          <ArrowDownToLine className="w-3 h-3 text-[#00F5FF]" />
          <span className="text-xs text-gray-400">Input Ports</span>
        </div>
        <div className="p-3 space-y-2">
          {inputPorts.map((p) => (
            <div key={p.port} className="bg-[#0d0d15] border border-[#1a1a2e] rounded-lg p-2">
              <div className="text-[10px] text-[#00F5FF]">Port {p.port}H</div>
              <div className="font-mono text-sm text-white">{p.value}H</div>
      ...
      <div className="bg-[#0a0a0f]/90 border border-[#1a1a2e] rounded-2xl overflow-hidden backdrop-blur-xl">
        <div className="flex items-center gap-2 px-3 py-2 border-b border-[#1a1a2e] bg-[#0d0d12]">
          ...
          <span className="text-xs text-gray-400">Output Ports</span>
        </div>
        <div className="p-3 space-y-2">
          {outputPorts.map((p) => (
            <div key={p.port} className="bg-[#0d0d15] border border-[#1a1a2e] rounded-lg p-2">
