    ...
    <div className="rounded-lg bg-[#0a0a0f] border border-white/5 overflow-hidden">
      ...
      <div className="p-3 grid grid-cols-2 gap-2">
        {statItems.map((item, idx) => (
          <div key={idx} className="bg-white/[0.02] border border-white/5 rounded p-2 flex flex-col items-center justify-center">
            <span className="text-[10px] text-gray-500 uppercase tracking-wider text-center">{item.label}</span>
            <span className="font-mono text-base font-semibold text-white mt-1">{item.value}</span>
          </div>
        ))}
      </div>
    </div>
