...
export default function BottomDock({ tabs, defaultTab, height = 220, className }: BottomDockProps) {
  ...
  return (
    <div
      className={cn(
        "border-t border-white/5 bg-[#08080c] flex flex-col",
        className
      )}
      style={{ height }}
    >
      {/* Tab Bar */}
      <div className="flex items-center gap-0 border-b border-white/5 bg-white/[0.01] shrink-0 overflow-x-auto">
        {/* Drag handle / label */}
        <div className="flex items-center gap-1.5 px-3 border-r border-white/5 shrink-0">
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex items-center gap-1.5 px-4 py-2 text-xs font-medium border-r border-white/5 transition-all duration-150 shrink-0 relative",
                isActive
